...
Status: implementation and deployment package prepared locally. No Kind cloud resources have been created. The live deployment and its runtime permissions are not yet verified.
...
1. Validate both templates with cfn-lint and the selected CloudFormation Guard rules. Finish a CloudFormation change set review before execution. Local validator installation is awaiting permission.
